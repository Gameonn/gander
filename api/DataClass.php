...
public static function get_pictures($uid,$owner_id,$zone,$user_date,$date){
...
$res2=$sth->fetchAll();
		...
		if(!ISSET($final[$value['id']])){
			...
			'phone_number'=>$value['phone_number']?$value['phone_number']:"",
			'user_pic'=>$value['photo']?BASE_PATH.$value['photo']:"",
		...
		$final[$value['id']]['pictures'][$value['pid']]=array(
			...
			"image_url"=>$value['image']?BASE_PATH.$value['image']:"",
...
public static function get_photos_count($app_status,$user_id,$owner_id,$zone,$user_date,$date){
...
public static function get_friend_pictures($uid,$owner_id,$zone,$user_date,$date){
...
where users.id=:owner_id AND users.id!={$uid} and photos.auto_approved=1 and (date_format(photos.date,'%m-%d')=date_format(('$date'),'%m-%d') or (DATE(photos.date) BETWEEN (('$date')- INTERVAL 6 DAY) AND ('$date'))) 
UNION
...
WHERE users.id=:owner_id AND photo_approved.status=1
  AND users.id!={$uid} AND (date_format(photos.date,'%m-%d')=date_format('$date','%m-%d') 
  OR (DATE(photos.date) BETWEEN ('$date' - INTERVAL 6 DAY) AND ('$date'))) AND (DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(photo_approved.created_on) +".SERVER_OFFSET."+ ({$zone}) ))=DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(UTC_TIMESTAMP) +".SERVER_OFFSET."+ ({$zone})   )))";
//
	$sth=$conn->prepare($sql);
	$sth->bindValue("owner_id",$owner_id);
	$sth->bindValue('user_id',$uid);
...
public static function get_friends_photos($contacts,$uid,$zone,$user_date,$date){
...
$res2=$sth->fetchAll();
			...
			if(!ISSET($final[$value['id']])){
				...
				//'photo_approved_status'=>$value['photo_approved_status']?1:0,
				'photo_approved_status'=>0,
				'photos_count'=>$value['photo_approved_status']?0:self::get_photos_count($value['photo_approved_status'],$uid,$value['id'],$zone,$user_date,$date),
				//'photos_count'=>$value['photo_approved_status']?0:$value['photos_count'],
				'pictures'=>self::get_friend_pictures($uid,$value['id'],$zone,$user_date,$date),
				);
			}
...
$res2=$sth->fetchAll();
	...
	if($res2){
		...
		'profile_pic'=>$res2[0]['photo']?BASE_PATH.$res2[0]['photo']:"",
...
$res2=$sth->fetchAll();
		...
		'name'=>$value['username']?$value['username']:"",
		'phone_number'=>$value['phone_number']?$value['phone_number']:"",
		'profile_pic'=>$value['photo']?BASE_PATH.$value['photo']:""
		);
		}}
...
$res2=$sth->fetchAll();
		...
		if($res2){
			foreach($res2 as $key=>$value){
				...
				'photo_id'=>$value['pid'],
				'photo'=>$value['image']?BASE_PATH.$value['image']:"",
				'date'=>$value['date'],
				'time_elapsed'=>$value[time_elapsed]?$value['time_elapsed']:0,
				'auto_approved'=>$value['auto_approved']
				);
