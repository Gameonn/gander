...
if(!($token)){
	...
	if(count($res)){
	$sql="select users.id as uid,users.username,users.photo,photos.*,photos.id as pid,Year(NOW())-Year(photos.date) as time_elapsed,photos.date as photo_date from date_request join users on user_id_requestor=users.id join photos on photos.user_id=date_request.user_id_owner and (date_format(photos.date,'%m-%d')=date_format(  '$date','%m-%d') or (DATE(photos.date) BETWEEN ('$date' - INTERVAL 6 DAY) AND ('$date'))) where user_id_owner=:uid and (date_format(FROM_UNIXTIME(UNIX_TIMESTAMP(date_request.created_on) +".SERVER_OFFSET."+ ({$zone})   ),'%m-%d')=date_format(FROM_UNIXTIME(UNIX_TIMESTAMP(UTC_TIMESTAMP()) +".SERVER_OFFSET."+ ({$zone}) ),'%m-%d')) and status=0";
	...
	$msg="Request Found";
		...
		if(!ISSET($final[$value['uid']])){
			...
			'pictures'=>DataClass::get_pictures($value['uid'],$uid,$zone,$user_date,$date)
			//'pictures'=>array()
			);
